...
- Sometimes, you may want to include all SQL files in a given directory.  For example, you may have
  ...
  element.  `includeAllSql` creates a changeSet for each SQL file it finds in a given directory.
  Each changeSet will contain a single `sqlFile` change that will rerun whenever the file itself
  changes.  The id of each generated changeSet will be derived from the name of the SQL file the
  changeSet runs.  The `includeAllSql` element takes a map of attributes that include the valid
  parameters from `includeAll`, `changeSet`, and `sqlFile`, with a few exceptions:
